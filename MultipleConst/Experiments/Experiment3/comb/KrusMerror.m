...
Str=["Penalty", "Feasibility", "Epsilon"];
   ...
   readFs.Penalty=csvread(strcat(Str(1), num2str(numFun), 'MerrorMultiple.csv'));
   readFs.Feasibility=csvread(strcat(Str(2), num2str(numFun), 'MerrorMultiple.csv'));
   readFs.Epsilon=csvread(strcat(Str(3), num2str(numFun), 'MerrorMultiple.csv'));
