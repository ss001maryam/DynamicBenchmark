...
MofPenalty=csvread(strcat(Str(1), num2str(numFun), 'MerrorMultiple.csv'));
MofFeasibility=csvread(strcat(Str(2), num2str(numFun), 'MerrorMultiple.csv'));
MofEpsilon=csvread(strcat(Str(3), num2str(numFun), 'MerrorMultiple.csv'));



    mp=mean(MofPenalty(:,1))
    mf=mean(MofFeasibility(:,1));
    me=mean(MofEpsilon(:,1));
    sp=std(MofPenalty(:,1));
    sf=std(MofFeasibility(:,1));
    se=std(MofEpsilon(:,1));
...
Latex =strcat(num2str(final(1)),'    &  ',num2str(mp),'($\pm$',num2str(sp),')',' & ', num2str(final(2)),'    &  ',num2str(mf),'($\pm$',num2str(sf),')','  & ', num2str(final(3)),'    &  ',num2str(me),'($\pm$',num2str(se),')')
