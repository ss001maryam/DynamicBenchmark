...
public class Penalty {
    ...
    public Penalty(int NP, int D, int numF, double CR, double [][][] vvv, int numCons) {
        this.NP = NP;
        this.numF = numF;
        this.D = D;
        this.CR = CR;
        ...
        for (int i = 0; i < this.NP; i++) {
            ...
            population.get(i).getFitness(this.numF, t,  vvv,numCons, true);
        }
        ...
        double pr=getpr();
        for (int i = 0; i < this.NP; i++) {
            population.get(i).getFitnessPenalty(numF, t,  vvv,numCons, fmin, fmax, pr, cmax);
        }
    }
...
//            System.out.println("g1 "+ g1[i]+ " g2 "+ g2[i]);
    ...
    public boolean detect_change(Individual ind,double [][][] vvv, int numCons) {
        ...
        ind.getFitness(numF, t, vvv,numCons, true);
        for (int k = 0; k < ind.g.size(); k++) {
            g_new[k] = ind.g.get(k);
        }
...
List <Double> getAverage(List<Double> values){
 ...
 population = new ArrayList();
    ...
    public static void main(String[] args) throws IOException {
      String[] Sfuncion = {"NumFunc_1", "NumFunc_2","NumFunc_3", "NumFunc_4"};
        ...
        int NP = 20, frequency = 1000, D = 30, eval, numVec = 3, gen, numCons=5;
 ...
 for (int i = 0; i < numChange; i++) {
            ...
            for (int r = 0; r < runs; r++) {
                ...
                Penalty DE = new Penalty(NP, D, numF, CR, vvv,numCons);// A random population was created
                    ...
                    for (int i = 0; i < DE.NP; i++) {
                        if (i == 0 || i == middleNP) { //the detection of change is carried out
                            if (DE.detect_change(DE.population.get(i), vvv,numCons)) {
                                //update the bestone 
                                bestOne.getFitness(DE.numF, DE.t,   vvv,numCons, true);                                
                                eval++;
                                ...
                                for (int j = 0; j < DE.NP; j++) {
                                    DE.population.get(j).getFitness(numF, DE.t, vvv,numCons, true);
                                    //Instead of update the bestone, we get the new bestone 
...
*/
                                ...
                                for (int j = 0; j < DE.NP; j++) {
                                    DE.population.get(j).getFitnessPenalty(numF, DE.t, vvv,numCons, fmin, fmax, pr, cmax);
                                }
                        ...
                        ind.getFitnessPenalty(numF, DE.t,   vvv,numCons, fmin,  fmax,  pr,  cmax);
